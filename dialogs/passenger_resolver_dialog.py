...
"""Handle passenger resolution for booking dialog."""
...
class PassengerResolverDialog(CancelAndHelpDialog):
    """Resolve the number of adult"""
    ...
    def __init__(
        ...
        super(PassengerResolverDialog, self).__init__(
            dialog_id or PassengerResolverDialog.__name__, telemetry_client
        )
        self.telemetry_client = telemetry_client

        passenger_prompt = TextPrompt(TextPrompt.__name__)
        passenger_prompt.telemetry_client = telemetry_client

        waterfall_dialog = WaterfallDialog(
            WaterfallDialog.__name__ + "2", [self.initial_step,
                                             self.analysis_adult_step,
                                             self.final_step]
        )
        waterfall_dialog.telemetry_client = telemetry_client

        self.add_dialog(passenger_prompt)
    ...
    async def initial_step(
        ...
        prompt_msg = "How many adults ?"
        reprompt_msg = (
            "I don't understand, please use syntax like '2 adults'"
        )
